...
| **opencode** | local or cloud | Hung on local Ollama at first, but that was mostly our config: use `127.0.0.1` (not `localhost`) + an `apiKey` placeholder, and a native-tool-calling model (qwen3-coder:30b works end to end; small coders connect but emit tool calls as text). See [OPENCODE-OLLAMA.md](OPENCODE-OLLAMA.md). |
